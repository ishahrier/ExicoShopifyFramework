...
{
    public class RefreshSettingsCache : IXCommand
    {
        public string GetName()
        {
            return "refresh-settings-cache";
        }

        public string GetDescription()
        {
            return "Reloads the settings from db to apps emmory cache.";
        }
        ...
        public async Task Run(XConsole xc)
        {
            try
            {
                ...
                using (var scope = xc.WebHost.Services.CreateScope())
                {
                    ...
                    {
                        xc.WriteInfo(this, $"Sending request to {remoteSite.Value} refresh cached settings list.");
                        var data = new WebClient().DownloadString($"{remoteSite.Value}/{XConsole.SERVICE_CONSTROLLER}/ReloadSettings?{config[AdminPasswordVerification.ADMIN_PASS_KEYT]}={config[AdminPasswordVerification.ADMIN_PASS_VALUE]}");
